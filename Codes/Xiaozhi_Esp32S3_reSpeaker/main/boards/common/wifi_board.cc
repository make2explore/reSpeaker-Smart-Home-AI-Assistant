...
static uint32_t GetLocalDeviceCode6() {
    // Deterministic 6-digit code derived from MAC suffix.
    // This is useful in headless/WiFi config mode where OTA activation code may not be available yet.
    ...
    std::string suffix = hex.substr(hex.size() - 6);
    uint32_t value = static_cast<uint32_t>(std::strtoul(suffix.c_str(), nullptr, 16));
...
void WifiBoard::EnterWifiConfigMode() {
    ...
    ESP_LOGW(TAG, "Device code (local, 6-digit): %06u", static_cast<unsigned>(GetLocalDeviceCode6()));
    ...
    // Also print the local 6-digit code in the spoken/displayed hint for headless setups
    char code_line[64];
    snprintf(code_line, sizeof(code_line), "Device code: %06u\n", static_cast<unsigned>(GetLocalDeviceCode6()));
    hint += code_line;
